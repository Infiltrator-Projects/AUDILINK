...
- exact AUDILINK revision and LINK dependency identity;
...
A successful build, unit test, simulator session or captured replay does not establish safe behaviour on every physical Audi/VAG vehicle.
...
A defect in generic LINK behaviour should be fixed in LINK and then consumed by AUDILINK; do not hide it with a product-only workaround unless necessary as an explicit temporary mitigation.
