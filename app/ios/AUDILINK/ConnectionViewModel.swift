...
struct AudiSavedVehicleSummary: Identifiable {
    let id: String
    let vin: String
...
@MainActor
final class ConnectionViewModel: NSObject, ObservableObject, @preconcurrency AudiLinkDiagnosticsControllerDelegate {
    ...
    @Published private(set) var savedVehicleProfiles = [AudiSavedVehicleSummary]()
    ...
    private let controller = AudiLinkDiagnosticsController()
    ...
    private let pollingSelectionStore = LinkPIDSelectionStore(
        productNamespace: "audilink-polling",
        legacyGlobalKey: nil,
        legacyVehicleKey: nil)
    ...
    override init() {
        ...
        if let value = audilink_version() { versionText = String(cString: value) }
        refresh()
    }
    ...
    func diagnosticsControllerDidUpdate(_ controller: AudiLinkDiagnosticsController) {
        refresh()
    }
    ...
    private func refreshSavedVehicleProfiles() {
        savedVehicleProfiles = vehicleProfileStore.savedProfiles.compactMap { profile in
            ...
            return AudiSavedVehicleSummary(
                id: vin,
                vin: vin,
                displayName: (profile["displayName"] as? String) ?? "Audi vehicle",
    ...
    private func saveVehicleProfile(vin: String) {
        ...
        if profile["displayName"] == nil { profile["displayName"] = "Audi vehicle · \(vin)" }
        profile["manufacturer"] = "Audi"
