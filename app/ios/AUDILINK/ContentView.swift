...
private enum ProductTheme {
    ...
    static let backgroundMiddle = Color(red: 0.055, green: 0.060, blue: 0.068)
    static let backgroundBottom = Color(red: 0.085, green: 0.090, blue: 0.10)
    static let panel = Color(red: 0.08, green: 0.09, blue: 0.10)
    static let panelRaised = Color(red: 0.12, green: 0.13, blue: 0.145)
    static let border = Color(red: 0.34, green: 0.35, blue: 0.37)
    static let accent = Color(red: 0.84, green: 0.07, blue: 0.17)
    ...
    static let fault = Color(red: 0.90, green: 0.18, blue: 0.25)
}
...
struct ContentView: View {
    ...
    private var aboutInfo: LinkDiagnosticAboutInfo {
        LinkDiagnosticAboutInfo(
            ...
            summary: "A C-first, open-source Audi vehicle diagnostics platform authored by Shannon Smith.",
            ...
            licenseText: "AUDILINK is free software licensed under the GNU General Public License version 3 or, at your option, any later version (GPL-3.0-or-later).\n\nSee LICENSE in the source package for the complete licence text.",
        ...
        LinkBrandHeader {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text("AUDI · LINK DIAGNOSTICS")
                        .font(.caption2.bold())
                        .tracking(1.2)
                        .foregroundStyle(ProductTheme.accent)
                    Text("Standards core with Audi-specific diagnostics layered on top")
                        .font(.caption)
                    ...
                    NavigationLink { ProductModulesView(model: model) } label: {
                        HStack {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text("Open standard responders and manufacturer capability details").font(.caption).foregroundStyle(ProductTheme.secondary)
                            }
                            Spacer()
                            Image(systemName: "chevron.right").foregroundStyle(ProductTheme.accent)
                        }
                    }
                    .buttonStyle(.plain)
                }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                LinkLabeledPanel(title: "Standard responders", systemImage: "square.stack.3d.up.fill") {
                    ...
                    Text("This is the LINK-owned standard responder inventory. Audi-specific module discovery is added only when it is evidence-backed.")
                        .font(.caption)
                        .foregroundStyle(ProductTheme.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                LinkLabeledPanel(title: "Graph", systemImage: "chart.xyaxis.line") {
                    ...
                    productDivider
                    if model.standardLiveRows.isEmpty {
                        Text("Collect live samples to populate graphable parameters.").font(.subheadline).foregroundStyle(ProductTheme.secondary)
                    } else {
                        ForEach(Array(model.standardLiveRows.prefix(6)), id: \.self) { row in
                            Text(row).font(.subheadline.monospacedDigit()).foregroundStyle(ProductTheme.primary)
                        }
                    }
            ...
            VStack(alignment: .leading, spacing: 15) {
                LinkLabeledPanel(title: "Readiness", systemImage: "checkmark.square.fill") {
                    ...
                    if model.readinessMonitorStatus.isEmpty {
                        Text("No readiness-monitor detail has been returned yet.").font(.subheadline).foregroundStyle(ProductTheme.secondary)
                    } else {
                        ForEach(model.readinessMonitorStatus, id: \.self) { Text($0).font(.subheadline).foregroundStyle(ProductTheme.primary) }
                    }
                }
                LinkLabeledPanel(title: "Freeze-frame context", systemImage: "camera.metering.matrix") {
                    if model.freezeFrameContext.isEmpty {
                        Text("No standard freeze-frame context captured.").font(.subheadline).foregroundStyle(ProductTheme.secondary)
                    } else {
                        ForEach(model.freezeFrameContext, id: \.self) { Text($0).font(.subheadline).foregroundStyle(ProductTheme.primary) }
                    }
                }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                LinkLabeledPanel(title: "Services", systemImage: "wrench.and.screwdriver.fill") {
                    Text(model.isActive ? "No verified service procedure is enabled for this session." : "Connect to evaluate supported service procedures.")
                    ...
                    Picker(
                        ...
                        ForEach(Array(model.languageTags.indices), id: \.self) { index in
                            let title = index < model.languageNames.count
                                ? model.languageNames[index] : model.languageTags[index]
                    ...
                    Picker(
                        ...
                        ForEach(Array(model.measurementKeys.indices), id: \.self) { index in
                            let title = index < model.measurementNames.count
                                ? model.measurementNames[index] : model.measurementKeys[index]
