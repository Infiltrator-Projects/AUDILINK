        ...
        LinkCommandCentreShell(
            ...
            .safeAreaInset(edge: .bottom, spacing: 0) {
                LinkDiagnosticAboutButton(
                    ...
                    copyright: "© 2026 Shannon Smith") { showingAbout = true }
                    .linkDiagnosticTheme(productTheme)
            }
        ...
        LinkBrandHeader {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("AUDILINK").font(.system(size: 29, weight: .bold)).tracking(1.2).foregroundStyle(ProductTheme.primary)
                    Text("AUDI · LINK DIAGNOSTICS").font(.caption2.bold()).tracking(1.2).foregroundStyle(ProductTheme.accent)
                    Text("LINK shared engine · Audi-specific knowledge layered above")
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Button {
                    ...
                    Label(model.isActive ? "Disconnect" : "Connect to vehicle",
                        ...
                        .padding(.vertical, 11)
                        .background(RoundedRectangle(cornerRadius: 12, style: .continuous).fill(ProductTheme.accent))
                }
                .buttonStyle(.plain)
                if model.isReady {
            ...
            VStack(alignment: .leading, spacing: 15) {
                ...
                LinkLabeledPanel(title: "Saved vehicles", systemImage: "car.2.fill") {
                    if model.savedVehicleProfiles.isEmpty {
                        Text("No saved Audi vehicles yet. A successful live VIN creates the profile automatically.")
                    ...
                    NavigationLink { ProductModulesView(model: model) } label: {
                        HStack {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text("LINK standard responders now; Audi-specific module knowledge remains product-owned.")
                                    .font(.caption).foregroundStyle(ProductTheme.secondary)
                            }
                            Spacer()
...
private struct ProductGraphView: View {
    ...
    private var graphable: [LinkDiagnosticParameter] {
        model.diagnosticParameters.filter { $0.vehicleSupported && $0.history.count > 1 }
    }
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 15) {
                LinkLabeledPanel(title: "Graph", systemImage: "chart.xyaxis.line") {
                    if graphable.isEmpty {
                        ...
                        ForEach(graphable.prefix(8)) { parameter in
                            ProductValueRow(label: parameter.title, value: "\(parameter.history.count) samples · latest \(parameter.presentationValue)", icon: "waveform")
                            Divider()
                        }
                    }
    ...
    var body: some View {
        ScrollView {
            LinkLabeledPanel(title: "Services", systemImage: "wrench.and.screwdriver.fill") {
                Text(model.isActive ? "No verified Audi-specific service procedure is enabled for this session." : "Connect to evaluate supported service procedures.")
                    .font(.headline).foregroundStyle(ProductTheme.primary)
                Text("Manufacturer procedures stay in AUDILINK; reusable execution and safety machinery stays in LINK.")
                    .font(.caption).foregroundStyle(ProductTheme.secondary)
            }.padding(16)
